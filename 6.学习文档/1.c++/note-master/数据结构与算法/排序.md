...
冒泡排序是一种相对较慢的排序，最佳情况时间复杂度为o（n），最坏时间复杂度为o（n^2^) 。平均时间复杂度是O(n^2^)
...
不管数组是否有序，最坏情况下o(n^2^),最好情况下o（n^2^）平均O(n^2^)
...
当选择“增量每次除以3”递减时，Shell排序的平均运行时间是O(nlog2n),最坏情况下o（n^2^）,最好情况下O（n）。
...
logn层递归中，每一层都需要O(n)的时间代价，因此总的时间复杂度是O(nlogn)，该时间复杂度不依赖于待排序数组中数值的相对顺序。因此，是最佳，平均和最差情况下的运行时间都为O(nlogn)
